...
function Recipes(props) {
  const [userId] = useState(props.auth.user.attributes.sub);
  const [userName] = useState(props.auth.user.username);
  const [reload, setReload] = useState(false);
  const [activeRecInst, setActiveRecInst] = useState("");
  const [recArr, setRecArr] = useState(props.appState.userRec);
  const [activeRecId, setActiveRecId] = useState("");
  const [defaultIng, setDefaultIng] = useState([...props.appState.defaultIng]); //set default ingredients list
  const [activeRecName, setActiveRecName] = useState("");
  const [activeRecIng, setActiveRecIng] = useState([]);
  const [activeRecNotes, setActiveRecNotes] = useState("");
  const [activeRecRating, setActiveRecRating] = useState(0);
  const [activeRecIndex, setActiveRecIndex] = useState(); // The index of the active recipe in recArr
  const [activeRecAuthorName, setActiveRecAuthorName] = useState("");
  const [recVolArr, setRecVolArr] = useState([]); //this is the size array for values converted to the user's preferred unit of measure.  
  const [ingWgtArr, setIngWgtArr] = useState ([]);//this is the size array for values converted to the user's preferred unit of measure.  Used in ingredient weight table calculations.
  const [recVolUOM, setRecVolUOM] = useState("");
  const [ingWgtUOM, setIngWgtUOM] = useState("");
  const [recSizeIndex, setRecSizeIndex] = useState(0);
  const [addRecIng] = useState("DEFAULT");
  const [qty, setQty] = useState(1); //quantity of drinks to be made at the user selected size

  useEffect(() => {
    console.log(activeRecIng);
    configDefaultIng();
    initializeUserSettings();
    
  },[reload]);
  
  let configDefaultIng = () => {
    ...
    let arr1 = [...props.appState.defaultIng];
    let arr2 = [...props.appState.userIng];
    let arr3 = [...activeRecIng];
    let addData = {
      //'editing': false,
      'ingUsed': false,
      'coeffLocked': false,
  ...
  function onChangeRecipeInst(e) {
    setActiveRecInst(e.target.value);
  }

  function onChangeRecipeName(e) {
    setActiveRecName(e.target.value);
  }

  function onChangeRecipeNotes(e) {
    setActiveRecNotes(e.target.value);
  }
  
  const handleSizeChange = (e) => {
    let index = e.target.getAttribute('data-key');
    setRecSizeIndex(index);
    //setReload(true);
  }
  ...
  const handleUpdateRecipe = async () => {
    ...
    let arr1 = [...activeRecIng];
    let arr2 = []; //An abbreviated recipe ingredient array.
    let newRec = activeRecId.length === 0; 
    ...
    try {
      const params = {
        "userId": userId,
        "userName": userName,
        "recId": activeRecId.length > 0 ? (activeRecId) : (null),
        "recName": activeRecName.length > 0 ? (activeRecName) : (null),
        "recIng": arr2,  
        "recDesc": activeRecNotes.length > 0 ? (activeRecNotes) : (null),
        "recInst": activeRecInst.length > 0 ? (activeRecInst) : (null)
      };
      const res = await axios.patch(`${config.api.invokeUrl}/recipes`, params);
      console.log(res.data);
      let data = res.data.Attributes;
      
      setActiveRecId(data.sk);
      setActiveRecName(data.iName);
            
      // subscribe user to drink if they are the creator
      if (newRec) {
        props.appState.subscribeRecipe( userId, data.sk, data.iName );
      }
  ...
  function deleteIng(e) {
    ...
    *  2) set the coeff of the deleted ingrdient to zero.
    ...
    */
    ...
    ingArr = distributeCoeff(ingArr, i, adjustedIngNewCoeff, step);
    setActiveRecIng(ingArr);
  }
  ...
  const getRecipe = async (e) => {
    ...
    try {
      ...
      setActiveRecName(data.iName);
      setActiveRecIndex(i);
      setActiveRecAuthorName(data.authName);
      setActiveRecNotes(data.text1);
      setActiveRecInst(data.text2);
      setActiveRecIng(data.ing);
      console.log(activeRecIng);
      setReload(!reload);
    }catch (err) {
  ...
  function addIng(e){
    ...
    let newArr = [...activeRecIng];
    let ingIndex = e.target.options[selectedIndex].getAttribute('data-key');
  ...
  return (
    <Container>
      <Col>
        ...
        <Nav vertical>
          <NavItem>
            <NavLink href="#" onClick={clearFields}><em>* create new *</em></NavLink>
          </NavItem>
          {recArr.map((obj,index) => {
              return (
                <NavItem key={index} >
      ...
      <Col>
        <Form>
          <FormGroup>
            ...
            <Input type="name" id='recipeName' value={activeRecName} onChange={onChangeRecipeName} placeholder="select a recipe or enter new name" />
          </FormGroup>
        
          <FormGroup>
            <Label for="recipeNotes">Description</Label>
            <Input type="textarea" id="recipeNotes" value={activeRecNotes} onChange={onChangeRecipeNotes} placeholder="enter description" />
          ...
          <FormGroup>
            ...
            <Input type="textarea" id='recipeInst' value={activeRecInst} onChange={onChangeRecipeInst} placeholder="type instructions here" />
          </FormGroup>
        
          <FormGroup tag="fieldset" >
            <Label for="recipeSize">Recipe Size ({recVolUOM})</Label>
            {recVolArr.map( (item, index) => {
              return (
          ...
          <FormGroup>
            <Input type="number" name="quantity" value={qty} onChange={handleQtyChange} min="1" />
          </FormGroup>
          ...
          <FormGroup>
            <Table size="sm">
              ...
              <tbody>
                {activeRecIng.map((obj,index) => {
                    if (obj.ingUsed === true) {
                      return (
                        <tr key={index} data-key={index}>
                          <td>{obj.iName + " " + obj.sett}</td>
                          <td>{Math.round(Number(ingWgtArr[recSizeIndex]) * qty * obj.coeff)} or {(Number(ingWgtArr[recSizeIndex]) * qty * obj.coeff).toFixed(2)}</td>
                          ...
                          <td><button key={index} data-key={index} onClick={deleteIng}>&times;</button></td>
                          <td><input type="checkbox" checked={obj.coeffLocked} key={index} data-key={index} onChange={toggleCoeffLock}></input></td>
                        </tr>
                      )
                    }
                  })
          ...
          <FormGroup>
            <Input type="select" value={addRecIng} onChange={addIng}>  //**************** may want to replace this with the react-select https://react-select.com/props
              <option value="DEFAULT" disabled>add ingredient</option>
              {activeRecIng.map((obj,index,array) => {
                  return (
          ...
          <Button block color="secondary" onClick={handleUpdateRecipe}>save</Button>
          <Button block color="warning" onClick={unsubscribe} disabled ={activeRecId.length === 0 ? ("disabled") : ("")}>delete</Button>
        </Form>
      </Col>
    </Container>
  );
